...
def plot_network(ifgdates, bperp, rm_ifgdates, pngfile, plot_bad=True):
   ...
   # fig = plt.figure(figsize=(18, 6))
    ...
    for i, ifgd in enumerate(ifgdates):
        ...
        label = '11 Connections' if i==0 else '' #label only first
        plt.plot([imdates_dt_all[ix_m], imdates_dt_all[ix_s]], [bperp[ix_m],
                bperp[ix_s]], color='b', alpha=0.6, zorder=2, label=label, linewidth=0.5)
    ...
    if plot_bad:
        for i, ifgd in enumerate(rm_ifgdates):
            ix_m = imdates_all.index(ifgd[:8])
            ix_s = imdates_all.index(ifgd[-8:])
            label = 'Descending 3* Connections' if i==0 else '' #label only first
            plt.plot([imdates_dt_all[ix_m], imdates_dt_all[ix_s]], [bperp[ix_m],
                    bperp[ix_s]], color='r', alpha=0.6, zorder=6, label=label, linewidth=1)
    ...
    try:  # Only support from Matplotlib 3.1
        ...
        for label in ax.get_xticklabels(fontsize=18):
    ...
    xlim_min = dt.datetime.strptime('20141001', '%Y%m%d')
    xlim_max = dt.datetime.strptime('20220401', '%Y%m%d')
    ...
    if np.all(np.abs(np.array(bperp))<=1): ## dummy
        plt.ylabel('dummy', fontsize=18)
    else:
        plt.ylabel('Bperp [m]', fontsize=18)
    
    #plt.legend()
    plt.title('028A_05385_191813 Network', fontsize=18, fontweight="bold", y=0.9)
